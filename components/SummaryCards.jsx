  ...
  return (
    <div className={`grid ${colMap[columns] || colMap[4]} gap-3 sm:gap-4 ${className}`}>
      {cards.map((card, i) => (
        <div
          key={i}
          className="bg-white rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all duration-200 p-4 sm:p-5 flex items-center gap-3 sm:gap-4 group"
        >
          {/* Icon block */}
          <div className={`
